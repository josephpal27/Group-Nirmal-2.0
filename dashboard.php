...
<head>
  ...
  <style>
    ...
    .sidebar .active {
      background-color: #3A739B !important;
      color: #fff;
    }
...
<body>
  ...
  <div class="sidebar">
    ...
    <?php if ($role == 'admin'): ?>
      <a href="dashboard.php?page=downloads" class="<?= $page == 'downloads' ? 'active' : '' ?>">
  ...
  <div class="main">
    <?php if ($page == ''): ?>
      ...
      <div class="row g-4">
        ...
        <?php if ($role == 'admin'): ?>
          <div class="col-md-3">
            <a href="dashboard.php?page=downloads" class="text-decoration-none">
              <div class="custom-card">
                <i class="bi bi-download"></i>
                <div class="custom-card-title">Downloads</div>
              </div>
            </a>
          </div>
        <?php endif; ?>
    ...
    <?php else:
      ...
      $role_permissions = [
        'admin' => [
          ...
          'add-blog',
          'edit-blog',
          'downloads' // ✅ Added downloads page for admin
        ],
        'blog' => ['blog-categories', 'blogs', 'add-blog', 'edit-blog'],
      ];

      // Check if the page exists in allowed permissions for the user's role
      if (isset($role_permissions[$role]) && in_array($page, $role_permissions[$role])) {

        // Build file path securely
        $file_path = __DIR__ . '/' . basename($page) . '.php';

        // Check if the file exists before including
        if (file_exists($file_path)) {
          include $file_path;
        } else {
          echo "<p>Page file not found.</p>";
        }
      } else {
