...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   $slug_input = mysqli_real_escape_string($conn, $_POST['slug']);
$slug = !empty($slug_input) 
    ? strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $slug_input), '-')) 
    : strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $title), '-'));

$update = "UPDATE blogs 
           SET title='$title', slug='$slug', content='$content', category_id=$category_id, image='$image',
...
<body class="container py-5">
  ...
  <input type="text" name="slug" class="form-control" placeholder="Enter custom slug or leave blank" value="<?= htmlspecialchars($blog['slug']) ?>">
</div>
